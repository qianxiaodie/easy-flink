  ...
  <div class="m-ide">
    <div class="m-ide-job" v-if="tabs.length">
      ...
      &lt;!&ndash; <div class="m-ide-toolbar">
        ...
        <u-text-button class="g-ml-16" @click="saveJob">
      ...
      <div class="m-ide-tabs">
        ...
        >
          <template v-slot:label="scope">
            <img v-if="scope.tab.type === 'SQL'" src="../../assets/images/icon_sqlTag.png" />
            <img v-else src="../../assets/images/icon_jarTag.png" />
            <span class="g-text-nowrap" :title="scope.tab.label" style="max-width: 250px;">{{ scope.tab.label }}</span>
          </template>
          &lt;!&ndash; 工具栏 &ndash;&gt;
          <div class="m-ide-toolbar">
            ...
            <u-text-button @click="releaseJob" :disabled="this.opCodesFlag === false">
              <img class="icon" src="../../assets/images/icon_shangxian.png" />
              <span>部署上线</span>
            </u-text-button>
            &lt;!&ndash; 运维 &ndash;&gt;
            &lt;!&ndash; 新增全屏按钮 &ndash;&gt;
            &lt;!&ndash; <div class="quanpin">
          ...
          <ndc-tab-panel class="m-ide-main" v-for="item in tabs" :key="item.name" :name="item.name">
            <m-ide-panel :ref="'panel_' + item.name" :job-id="item.name" @debug="onDebugChange" @ide-modified="onIdeModified"></m-ide-panel>
          </ndc-tab-panel>
        </ndc-tabs>
      </div>
    </div>
    ...
    <div v-else class="m-ide-new">
      ...
      <div class="new-item-box ">
        <div class="new-item new-item-sql">
          ...
          <ndc-button theme="active" @click="createJob('SQL')">新建SQL任务</ndc-button>
        </div>
...
export default class MIde extends Vue {
  ...
  private async showReleaseJob() {
    ...
    if (result.opCodes.includes('rtdev_proj.devdeploy') || result.opCodes.includes('rtdev_proj.devtest')) {
  ...
  //     if (result.opCodes.includes('rtdev_proj.devdeploy') === true) {
  //       this.opCodesFlag = true;
  //     }
  //   }
  ...
  private async copyJob() {
    try {
      const jobDetail: IJob = await this.$request('getJobDetail', { jobId: this.activeTab }); // 获取当前任务信息
