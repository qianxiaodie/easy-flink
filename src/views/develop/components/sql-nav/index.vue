    ...
    <m-nav-header placeholder="任务名称" @refresh="getJobList" @tags-change="selectedTags = arguments[0]" @keywords-change="keywords = arguments[0]">
      ...
      <template v-slot:left>
        <ndc-tooltip position="top" content="新建文件夹">
          <img :class=[{changeCursor:!opCodesFlag},{newfloder:opCodesFlag}] src="@/assets/images/新建文件夹.png" alt @click="createFolder(true)"/>
        </ndc-tooltip>
        <div class="icon-line"></div>
        <ndc-tooltip position="top" content="新建SQL任务">
          <img src="@/assets/images/新建任务.png" alt @click="createJob('SQL')" :class=[{changeCursor:!opCodesFlag},{newSqlTask:opCodesFlag}] />
        </ndc-tooltip>
        <div class="icon-line"></div>
      </template>
...
<!--          <img class="dingwei" src="@/assets/images/定位任务.png" alt @click="debugJob" />-->
        </ndc-tooltip>
      </template>
    </m-nav-header>
...
export default class MSqlNav extends Mixins(navMixin) {
  ...
  @Action('getMetadataTableList', MODULE_NAMESPACE.develop)
  ...
  private create(data: any, isCopy: boolean = false) {
    ...
    this.jobModalInstance &&
      this.jobModalInstance.show(
        this.isRightSql ? Object.assign({}, data, {
  ...
  private isRightSql:boolean =false
    ...
    if (!this.folderContextMenuInstance) {
      this.folderContextMenuInstance = this.$contextmenu(folderMenuItems, (item: IMenuItem, event: Event) => {
        ...
        if (item.key === 'createSQL') {
          // 注意 this.isRightSql = false 的位置 ★
          this.isRightSql = true;
          return this.createJob('SQL');
        }
      });
    }
