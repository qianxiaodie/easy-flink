<template>
  <div class="m-run-params">
    <ndc-form>
      ...
      <span v-show="job.jobType === 'JAR' && !job.flinkJarPath" style="color: #f04346; font-size: 12px; margin-left: 51px">请先上传Jar包，再修改参数！</span>
      <ndc-form-item label="内核版本：">
        ...
        <ndc-select
          ...
          :disabled="job.jobType === 'SQL' ? true : !opCodesFlag || !job.flinkJarPath"
        ></ndc-select>
      </ndc-form-item>
...
export default class MRunParams extends Vue {
  ...
  /* private mounted() {
    if (this.job.jobType === 'JAR' && !this.job.flinkJarPath && !this.job.flink) {
      this.opCodesFlag = false;
      this.$ndcmessage.warning('请先上传Jar包，再修改参数！')
      /!*this.$ndcconfirm({
        content: '请先上传jar包',
        cancelButton: ''
      });*!/
      console.log(this.job,'onClickBeforeShow')
      // return
    }
  } */
