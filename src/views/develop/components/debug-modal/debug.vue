        ...
        <div class="m-debug-body">
          <div class="body-row g-flex-ac" v-for="(tbl, index) in sourceTables" :key="index">
            <u-validator class="g-flex-ac" style="width: 100%;" :name="'srcerror_' + index" :validate="errors">
              <div class="g-text-nowrap body-row-td" style="width: 38%;" :title="tbl.tableName">{{ tbl.tableName.length > 20 ? tbl.tableName.substring(0,20) + "..." : tbl.tableName}}</div>
    ...
    if (files.length) {
      ...
      if (/^[a-zA-Z0-9_]*$/.test(name) && name.length <= 64) {
        ...
        try {
          ...
          // this.addValidatorError(field, '文件上传失败');
          this.$handleException(e, '文件上传失败');
        } finally {
