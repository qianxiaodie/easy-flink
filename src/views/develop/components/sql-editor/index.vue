<template>
  <div class="m-sql-editor" style="width:100%;height: 100%" :readOnly="true"></div>
</template>
...
export default class MSqlEditor extends Vue {
  ...
  // 获取用户权限操作码列表
  // @State('opCodes', MODULE_NAMESPACE.develop)
  // private opCodes!: any[]
  ...
  private async mounted() {
    ...
    if (!this.options.readOnly) {
      ...
      // console.log(this.opCodes,'opcodes-state')
      // console.log(result,'response-sqleditor')
      // debugger
      // if (result.opCodes.includes('rtdev_proj.devtest') === false) {
      //   EDITOR_OPTIONS.readOnly = true;
      // }
      await getOpCodes({}).then(response => {
        console.log(response,'response-sqleditor')
        if (response.opCodes.includes('rtdev_proj.devtest') === false) {
          EDITOR_OPTIONS.readOnly = true;
        }
      });
    }
    // console.log(this.value,'sql编译器内容')
    const editor = (this.editor = monaco.editor.create(this.$el as HTMLElement, Object.assign({}, EDITOR_OPTIONS, this.options)));
    this.addCommand();
    // console.log(this.value,'sql编译器内容')
